...
page_3d_rose,
page_3d_ico,
# page_highlight.htm, # meg kell írni
# page_3d_sketches,
# page_3d_sketch,
# page_3d_fractal,
page_3d_export,
# page_sketch_mode,
manual17, # moved at the end index
# page_plot_search.htm # csak egy mondat
);

@month = qw( Január Február Március Aprilis Május Június Július Augusztus Szeptember Október November December );
($sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst ) = localtime();
$year += 1900;

open( DAT, "../TDM/AndroidManifest.xml" );
while ( $line = <DAT> ) {
...
foreach $i (@html) {
  ...
  while ( $line = <DAT> ) {
    if ( $line =~ /<!-- NOW -->/ ) {
      print OUT "Verzió $version -  $year $month[$mon] $mday.  <br>";
      next;
    }
    if ( $line =~ /<html>/ ) {
      print OUT "<a name=\"$i\" />\n";
