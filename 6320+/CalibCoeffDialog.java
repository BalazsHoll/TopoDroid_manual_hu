...
import com.topodroidhb.TDX.GMActivity;
                       ...
                       implements View.OnClickListener
{
 ...
 /** cstr
  ...
  static Bitmap makeRotBitmap(float[] rot, float[] azi, float[] inc, int width, int height, int bin, int step, int col ) // HBX
  {
    ...
    int sector=0;
    if (azi[0]<45.0f) sector=1;
    if (azi[0]>315.0f) sector=3;
    maxa = azi[0];
    mina = azi[0];
    maxi = inc[0];
    mini = inc[0];
    for ( int k=1; k < rot.length; ++ k ) {
      if ((sector == 1) && (azi[k]>315.0f)) azi[k]-=360.0f;
      if ((sector == 3) && (azi[k]<45.0f)) azi[k]+=360.0f;
