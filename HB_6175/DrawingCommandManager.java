...
import com.topodroidhb.utils.TDColor; // HBX
...
*/
    ...
    float pscale = TDSetting.mToPdf; // HBX
    if ( zoom < 0 ) { // PDF print //75
      PDF_page = true;
      mm = new Matrix();
      // scale = 1.0f; // getBitmapScale();

      //bbox  = getBitmapBounds( 1.0f);
      bbox  = getBitmapBounds( pscale );
      //bbox = new  RectF(bbox.left-10, bbox.top-10, bbox.right+20-100,bbox.bottom+20-120); // HBX
      TDLog.v(" HBE1 scale " + scale + " bbox " + bbox.left + " " + bbox.top + " " + bbox.right + " " + bbox.bottom + " zoom " + zoom );
      // float sca = 1 / pscale
      //mm.postTranslate( 40 - bbox.left/scale, 40 - bbox.top/scale ); //MC
      //mm.postScale( scale, scale ); //MC
      //mm.postTranslate( 00.0f*scale - bbox.left/scale, 00.0f*scale - bbox.top/scale );
      // mm.postScale( scale, scale );
      // mm.postTranslate( 40.0f*scale - bbox.left, 40.0f*scale - bbox.top );
//      mm.postTranslate( 10 - bbox.left-100, 10 - bbox.top-120 );
//      bbox = new  RectF((bbox.left-10)/pscale, (bbox.top-10)/pscale, (bbox.right+100+20)/pscale,(bbox.bottom+120+20)/pscale); // HBX
      int margin_left = 40, margin_right=40, margin_top=40,margin_bottom=40;
//      bbox = new  RectF((bbox.left-10)/pscale, (bbox.top-10)/pscale, (bbox.right+100+20)/pscale,(bbox.bottom+120+20)/pscale); // HBX
      mm.postTranslate( margin_left - bbox.left/pscale, margin_top - bbox.top/pscale ); // HBX
//      mm.postTranslate( 40 - bbox.left/pscale, 40 - bbox.top/pscale ); // 6.1.76
      mm.postScale( pscale, pscale );
      ...
      zoom = -zoom * pscale;
      sidebars = false; // do not draw sidebars
      //grids    = false; //HBX common grids setting
      TDLog.v(" HBE2 pscale " + pscale + " bbox " + bbox.left + " " + bbox.top + " " + bbox.right + " " + bbox.bottom + " zoom " + zoom );
    }
    ...
    synchronized( TDPath.mGridsLock ) {
      if( grids && mGridStack1 != null ) {
        ...
        if ( isFixedZoom() ) {
          ...
          int j = - (int)( mOffy / step ); // HBX
          float y = mOffy +  j * step ; // HBX
          ...
          for ( ; x<(TopoDroidApp.mDisplayWidth+400); x += step ) { // HBX
            ...
            if ( i % 10 == 0 ) {
              ...
              //HBX dpath.setPathPaint( paint_grid );
              dpath.setPathPaint( fixedGridStepxPaint ); // HBX
            }
            ++i;
          ...
          for ( ; y<(TopoDroidApp.mDisplayHeight+400); y += step ) { //HBX
            ...
            if ( j % 10 == 0 ) { 
              ...
              //HBX dpath.setPathPaint( paint_grid );
              dpath.setPathPaint( fixedGridStepyPaint ); // HBX
            }
            ++j;
            dpath.mPath  = new Path();
      ...
      if ( mNorthLine != null ) {  // ez mi ehet? HBX
        if ( inverted_colors ) {
        ...
        if ( inverted_colors ) {
          if ( sidebars ) {
            ...
            if (PDF_page) { // HBX 20-> PDF 1/72 inch
              mScaleRef.draw(canvas, zoom, mLandscape, 20, (bbox.bottom - bbox.top)*pscale-20, sketch_unit, 0xffffff );
            } else {
          ...
          if ( sidebars ) {
            ...
            if (PDF_page) {
              mScaleRef.draw(canvas, zoom, mLandscape, 20, (bbox.bottom - bbox.top)*pscale-20, sketch_unit );
            } else {
