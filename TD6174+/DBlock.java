...
public class DBlock
{
   ...
   * @param show_id   whether to include the block ID
   */
  public String toShortStringRoll( boolean show_id ) // HBX
  {
    float ul = TDSetting.mUnitLength;
    float ua = TDSetting.mUnitAngle;
    StringWriter sw = new StringWriter();
    PrintWriter pw  = new PrintWriter(sw);
    if ( show_id ) pw.format("%d ", mId );
    pw.format(Locale.US, "<%s-%s> %.2f %.1f %.1f %.1f", mFrom, mTo, mLength*ul, mBearing*ua, mClino*ua, mRoll*ua );
    return sw.getBuffer().toString();
  }
  ...
  String dataStringRoll( String fmt ) //  HBX
  {
    float ul = TDSetting.mUnitLength;
    float ua = TDSetting.mUnitAngle;
    return String.format(Locale.US, fmt, mLength*ul, mBearing*ua, mClino*ua, mRoll*ua );
  }
