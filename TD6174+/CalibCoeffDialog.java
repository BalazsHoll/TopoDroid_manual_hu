...
import com.topodroidhb.math.TDMatrix;
import com.topodroidhb.math.TDVector;
import com.topodroidhb.ui.MyDialog;
// import com.topodroid.prefs.TDSetting;
import com.topodroidhb.TDXHB.R;
import com.topodroidhb.TDXHB.GMActivity;
                       ...
                       implements View.OnClickListener
{
  ...
  static Bitmap makeHistogramBitmap( float[] error, int width, int height, int bin, int step, int col )
  {
    ...
    return bitmap;
       ...
       // hist3.setImageBitmap( CalibCoeffDialog.makeRotBitmap( rot1, azi1, inc1, 400, 100, 40, 60, TDColor.FIXED_ORANGE ) ); // HBX

  static Bitmap makeRotBitmap( float[] rot, float[] azi,float[] inc, int width, int height, int bin, int step, int col ) // HBX
  {
    Bitmap bitmap = Bitmap.createBitmap( width+20, height+20, Bitmap.Config.ARGB_8888 );
    int ww = bitmap.getWidth();
    int hh = bitmap.getHeight();
    for ( int j=0; j<hh; ++j ) {
      for ( int i=0; i<ww; ++i ) bitmap.setPixel( i, j, 0 ); // clear bitmap
    }
    ...
    int joff = hh-10;
    int ioff = 10;
    int dx   = ww / bin; // ww=420 bin=40 dx=10
    if ( dx*20 >= ww ) dx --;
    int x, y;
    ...
    maxi = inc[0];
    mini = inc[0];
    for ( int k=1; k < rot.length; ++ k ) {
      if (maxa < azi[k]) maxa = azi[k];
      if (mina > azi[k]) mina = azi[k];
      if (maxi < inc[k]) maxi = inc[k];
      if (mini > inc[k]) mini = inc[k];
    }
    ka = (maxa + mina)/2;
    ki = (maxi + mini)/2;
    da = (maxa - mina);
    ...
    for ( int k=0; k < rot.length; ++ k ) {
      for ( int x1=0; x1 < 4; ++ x1 ) {
        for ( int y1=0; y1 < 4; ++ y1 ) {
          bitmap.setPixel((int) (rot[k]) + x1+10, (int) ( ((azi[k]) - (ka))/da * 80 + 40 + y1 ), TDColor.FULL_RED);
        }} // 4x4 pixel azi

      //TDLog.v("Graph Rot "+rot[k]);
      //TDLog.v("Graph Azi "+azi[k]);
      //TDLog.v("Graph Inc "+inc[k]);
      for ( int x1=0; x1 < 4; ++ x1 ) {
        for ( int y1=0; y1 < 4; ++ y1 ) {
          bitmap.setPixel((int) (rot[k]) + x1+10, (int) ( ((inc[k]) - (ki))/di * 80 + 60 + y1 ), TDColor.FIXED_YELLOW);
        }} // 4x4 pixel inc
    }

    for ( int x1=0; x1 < 10; ++ x1 ) {
      for ( int y1=0; y1 < 2; ++ y1 ) {
        bitmap.setPixel((int) (360) + x1+10, (int) ( (0.5f)/da * 80 + 40 + y1 ), TDColor.FULL_RED);
        bitmap.setPixel((int) (360) + x1+10, (int) ( (0.0f)/da * 80 + 40 + y1 ), TDColor.FULL_RED);
        bitmap.setPixel((int) (360) + x1+10, (int) ( (-0.5f)/da * 80 + 40 + y1 ), TDColor.FULL_RED);
        bitmap.setPixel((int) (-10) + x1+10, (int) ( (0.5f)/di * 80 + 60 + y1 ), TDColor.FIXED_YELLOW);
        bitmap.setPixel((int) (-10) + x1+10, (int) ( (0.0f)/di * 80 + 60 + y1 ), TDColor.FIXED_YELLOW);
        bitmap.setPixel((int) (-10) + x1+10, (int) ( (-0.5f)/di * 80 + 60 + y1 ), TDColor.FIXED_YELLOW);
      }} // 10x2 pixel azi, inc scale

    for ( y = 0; y < hh; ++y ) bitmap.setPixel( ioff, y, red ); // y axis
    for ( x = 0; x < ww; ++x ) bitmap.setPixel( x, joff, red ); // x axis
    for ( int k = 3; k <= 36; k+=3 ) { //3*10 grad
      x  = ioff + dx * k;
      int yy = hh - ( ((k%9) == 0 )? 0 : 5 ); // 90 deg
      for ( y = joff; y < yy; ++y ) bitmap.setPixel( x, y, red );
    }
    if ( 5  <= bin ) {
      x  = 180+10;
    ...
    for ( int k = 10; ; k += 10 ) {
      y = joff - step * k; //joff=hh-10 step=50
      if ( y < 0 ) break;
      for ( x = 5; x < ioff; ++x ) bitmap.setPixel( x, y, red );
    }
