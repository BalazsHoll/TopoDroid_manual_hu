...
import com.topodroidhb.TDXHB.SurveyWindow;
...
// -------------------------------------------------------------------
  ...
  public void onItemClick(AdapterView<?> parent, View view, int position, long idx )
  {
    ...
    if ( blk != null ) {
      ...
      float[] azi1 = new float[n1]; // aimuth gegree
      float[] inc1 = new float[n1]; // inclination degree
      ...
      for ( k = 0; k<n1; ++k ) {
        ...
        TDLog.v("Graph Roll "+b.mRoll);
        rot1[k] = (float)(b.mRoll); // HBX
        azi1[k] = (float)(b.mBearing);
        inc1[k] = (float)(b.mClino);
      }
